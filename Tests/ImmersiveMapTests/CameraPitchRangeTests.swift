...
final class CameraPitchRangeTests: XCTestCase {
    ...
    /// The globe's ceiling is the flat map's fixed one at every zoom, so the
    /// still recorder's default state, which the floor lifts to a tilt, keeps
    /// that tilt when the surface constraints are applied at zoom 0.
    func testZoomedOutGlobeConstraintKeepsTheFlooredDefaultState() {
