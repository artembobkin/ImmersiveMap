...
final class TileMaximumZoomLevelSettingsTests: XCTestCase {
    ...
    /// An archive built to a different depth than the default states it with
    /// one modifier next to its URL, in either direction.
    ...
    func testCoverageResolutionHonorsTheStatedDepth() {
        ...
        XCTAssertEqual(defaultTiles.resolvedCoverageZoomLevel(forCameraZoom: 14.4), 14)
        XCTAssertEqual(defaultTiles.resolvedCoverageZoomLevel(forCameraZoom: 15.0), 15)
