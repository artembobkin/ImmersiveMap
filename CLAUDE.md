...
- Do not push. Prepare the branch, commit, then stop and report what is ready and what the push would carry. `git push` and opening the pull request happen only when the maintainer asks for them in that message; the permission covers that push alone and does not carry over to the commits that follow it. This holds even if a push was mentioned as the plan earlier in the conversation, and `.claude/settings.json` also makes every push ask.
