...
- Never edit the documentation (`Documentation/`, `README.md`, `CHANGELOG.md` and every other `.md` file except this one and the rules under `.claude/`). When a change makes a documented statement wrong, leave the file as it is and say which file and which statement in the report. The person at the screen updates the documentation.
