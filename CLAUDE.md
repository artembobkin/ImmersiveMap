...
- A pull request gets at most two review cycles, where a cycle is one round of waiting for feedback, verifying it, and pushing fixes. If the review is still asking for changes after the second cycle, stop: do not open a third round, do not merge, and do not keep polling. Report instead, and say what the actual problem is: which remarks keep coming back, whether they are confirmed by the code or wrong, and what is blocking a resolution (a real defect that needs a decision, a disagreement about the approach, a check that will not go green, a review asking for something outside the task). Two cycles that do not converge mean the work needs a decision, not another round.
