...
- Routes on the globe, and 3D models that fly along them. `ImmersiveMapGeoPath` describes a trajectory as waypoints joined by great-circle arcs plus an altitude profile (`base + peak * sin(pi * fraction)`, so a flight path arcs away from the planet and back), parameterized by arc length rather than by waypoint index. The same value feeds both new pieces, which is what guarantees the model rides exactly on the line that is drawn. `ImmersiveMapRoutesController` and the `.routes(_:)` modifier draw a route as a ribbon in the map world pass with depth testing, so the far half of an arc disappears behind the globe and a model in front of the line covers it; the width is given in points and is expanded in the vertex shader, so it stays constant on screen at any zoom and through globe rotation, with edges antialiased by the same signed-distance convention the text and marker shaders use. `progress` truncates the ribbon at an exact arc-length fraction (not at the nearest tessellated point), and `setProgress(id:_:duration:)` eases it for a line that draws itself. On the scene model side, `animate(id:along:duration:curve:appliesHeading:appliesPitch:completion:)` moves an existing model along the path over a real duration, with the course taken from the trajectory tangent and the pitch from the altitude gradient, so a plane banks into the climb and noses down on descent; `completion` fires exactly once on the main thread, `true` on arrival and `false` when the animation is superseded, cancelled through `cancelPathAnimation(id:)`, or dropped because the model was removed, the view went away, or a settings change recreated the renderer, so journey chains never hang. While a path animation runs it owns the model's coordinate, altitude, heading and pitch; competing `move` / `setAltitude` / `setOrientation` calls for that id are recorded but not applied. Internally the sphere-to-plane morph is now evaluated once, on the CPU, by a shared `GeoSurfaceFrameMath` extracted from the scene model anchor path, so the ribbon centerline and the model anchor cannot drift apart; the route shader only widens the centerline. Known limits in this version: routes draw in globe presentation only (including the whole morph, fading out over its last tenth) and are absent on the fully flat map, the model animation is not restricted that way; solid lines only, without dash patterns or gradients; routes are excluded from tour video export; and a route with a zero altitude profile can stipple against the surface, so give ground tracks a small `baseAltitudeMeters`. See `Documentation/docs/routes.md`.
