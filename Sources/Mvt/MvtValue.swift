...
/// table (`MvtAttributeDecoder`).
package enum MvtValue: Hashable, Sendable {
    case string(String)
    case float(Float)
    case double(Double)
    case int(Int64)
    ...
    package var uintValue: UInt64? {
        if case .uint(let value) = self { return value }
        return nil
    }

    package var sintValue: Int64? {
...
package enum MvtGeometryType: Int, Hashable, Sendable {
    case unknown = 0
    case point = 1
    case linestring = 2
    case polygon = 3
