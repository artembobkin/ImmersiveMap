...
Without the modifier the map renders the hosted `ImmersiveMapTilesService` source. A source that does not ship the default z0-14 states its depth via `tileSettings` (`tiles.coverage.maximumZoomLevel`). For full control (a base URL with TileJSON discovery instead of a fixed template), the same fields live on `ImmersiveMapSettings.TileSettings.NetworkSettings`, see [tile cache](tile-cache.md).
