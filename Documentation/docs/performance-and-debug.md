...
The tile list reserves a fixed number of slots, so its height never changes and nothing under it moves when a tile arrives, leaves or is expanded; rows that do not fit are counted on a final `+N more` line. It is also last in the column, so an expanded tile spends slots on its children without disturbing anything.
...
The **Shadows** group edits the live [shadow settings](buildings-and-shadows.md): on/off, strength, shadow map size, coverage, the normal offset, the caster-height limit, and the sun's azimuth and elevation. They are applied the way any settings change is, so no renderer is rebuilt for a drag, and what the panel is dragged to holds: it rides on top of the settings the app supplies, because SwiftUI re-sends its own value on every update of the view hierarchy and would otherwise revert a slider as soon as anything else on screen changed. The panel wins only while it is on; turning it off drops what was dragged and the app's own values come straight back.
