...
The default map is a light, warm, low-contrast palette in the manner of the system maps people already know: a warm off-white ground, soft pastel greens for parks and forests, a clear light blue for water, and asphalt-grey streets in the manner of a driving map, each with a slightly deeper edge, in a road hierarchy that reads as depth of grey (a mid asphalt on the motorways down to pale grey minor streets that stay a quiet step under the ground). Buildings are a warm light grey a step under the ground, so a roof separates from the street around it and the renderer's wall shading separates the walls from the roof. The overview biomes of the globe are the same colors, class by class, so nothing shifts hue while zooming.
