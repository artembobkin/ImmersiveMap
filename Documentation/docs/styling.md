...
The default map is a light, warm, low-contrast palette in the manner of the system maps people already know: a warm off-white ground, soft pastel greens for parks and forests, a clear light blue for water, and asphalt-grey streets in the manner of a driving map: one grey road surface across the whole automobile network, each road with a slightly deeper edge, in a hierarchy that reads as width alone (wide motorways down to narrow service alleys). From street zoom a road is drawn at its real carriageway width, taken from the lane count the tiles carry, so a six-lane avenue is six lanes wide; the asphalt stays bare, with the measured lane paint and carriageway surfaces available as the opt-in [streetscape](streetscape.md). Buildings are a warm light grey a step under the ground, so a roof separates from the street around it and the renderer's wall shading separates the walls from the roof. The overview biomes of the globe are the same colors, class by class, so nothing shifts hue while zooming.
