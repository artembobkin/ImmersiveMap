...
**The streetscape, opt-in.** The map tiles draw roads as a street map does, casing and fill by class with nothing painted on them. The measured streetscape, the carriageway surfaces and the lane paint, is a second archive the service serves next to the map tiles at street zoom, requested only with `.streetscape(isEnabled: true)`; see [the streetscape](streetscape.md).
