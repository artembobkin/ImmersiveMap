...
It is data, not a setting. Where a tile's road layer carries it, the map draws the roads as carriageways at their real width, flush with the surfaces, with the paint on top. Where a tile carries none, the same style draws a street map: the roads are lines only, strokes by class with a casing, and nothing else, no crossing stripes, no parking lots, no road polygons. There is nothing to switch on and no second request: one tile, drawn for what it contains.
