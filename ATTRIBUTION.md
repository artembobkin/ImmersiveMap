...
**Keep the map data credit visible wherever the map is.** The engine already does it, so a plain map needs nothing from you.
...
**The engine gives you a compliant default.** A stock `ImmersiveMapView()` renders the badge for the active provider with no configuration at all, so if you write nothing, you are covered.

**The app owns the obligation.** The moment you hide, empty or override the badge, compliance moves to you: the license binds the product that ships the map, not the library that draws it. The engine helps as much as a library can, warning when a map starts without a visible credit, but it cannot see your other UI, and it does not try to.
...
Once your app does show the credit on its own, in a custom overlay or an about screen, declare it, and the warning goes away:
