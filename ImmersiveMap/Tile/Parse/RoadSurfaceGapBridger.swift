...
/// surface and enters another within a few metres, the piece between the two
/// cut points is a slit, and the outline edges the cut points lie on are the
/// trim edges. The line, and both surfaces, must carry the SAME street
/// identity (the `street` attribute, an id assembled from the whole road
/// network): that requirement is what keeps the bridger off the median of a
/// dual carriageway, whose two opposing one-way roads carry different ids,
/// and off every gap a crossing footpath threads between two unrelated
